...
#define TORCH_CHECK(cond, ...) GGML_ASSERT(cond)
#define TORCH_CHECK_NOT_IMPLEMENTED(cond, ...) GGML_ASSERT(cond)
...
static vllm::ScalarType ggml_cuda_marlin_type_to_scalar(ggml_cuda_marlin_type type) {
    switch (type) {
        case GGML_CUDA_MARLIN_TYPE_S4:      return vllm::kS4;
        case GGML_CUDA_MARLIN_TYPE_U4:      return vllm::kU4;
        case GGML_CUDA_MARLIN_TYPE_U4B8:    return vllm::kU4B8;
        case GGML_CUDA_MARLIN_TYPE_S8:      return vllm::kS8;
        case GGML_CUDA_MARLIN_TYPE_U8:      return vllm::kU8;
        case GGML_CUDA_MARLIN_TYPE_U8B128:  return vllm::kU8B128;
        case GGML_CUDA_MARLIN_TYPE_FE2M1F:  return vllm::kFE2M1f;
        case GGML_CUDA_MARLIN_TYPE_FE4M3FN: return vllm::kFE4M3fn;
        case GGML_CUDA_MARLIN_TYPE_FE8M0FNU:return vllm::kFE8M0fnu;
        case GGML_CUDA_MARLIN_TYPE_F16:     return vllm::kFloat16;
        case GGML_CUDA_MARLIN_TYPE_BF16:    return vllm::kBFloat16;
        default:                            return vllm::kFloat16;
    }
}

static ggml_cuda_marlin_type ggml_cuda_marlin_type_from_ggml(enum ggml_type type) {
    switch (type) {
        case GGML_TYPE_F16:  return GGML_CUDA_MARLIN_TYPE_F16;
        case GGML_TYPE_BF16: return GGML_CUDA_MARLIN_TYPE_BF16;
        default:
            GGML_ABORT("unsupported ggml type for marlin: %s", ggml_type_name(type));
    }
}

extern "C" int ggml_cuda_marlin_min_workspace_elements(int device) {
    int sms = 0;
    CUDA_CHECK(cudaDeviceGetAttribute(&sms, cudaDevAttrMultiProcessorCount, device));
    return sms;
}

extern "C" bool ggml_cuda_marlin_gemm(const ggml_cuda_marlin_gemm_params * params) {
    ...
    const int sms = params->sms > 0 ? params->sms : ggml_cuda_marlin_min_workspace_elements(device);
    const auto a_type = ggml_cuda_marlin_type_to_scalar(params->a_type);
    const auto b_type = ggml_cuda_marlin_type_to_scalar(params->b_type);
    const auto c_type = ggml_cuda_marlin_type_to_scalar(params->c_type);
    const auto s_type = ggml_cuda_marlin_type_to_scalar(params->s_type);
    ...
    marlin::marlin_mm(
            ...
            params->c,
            params->c_tmp,
            const_cast<void *>(params->b_bias),
            const_cast<float *>(params->a_scales),
            const_cast<void *>(params->b_scales),
            const_cast<void *>(params->global_scale),
            const_cast<void *>(params->b_zeros),
            const_cast<int *>(params->g_idx),
            const_cast<int *>(params->perm),
            params->a_tmp,
            int(params->size_m),
            int(params->size_n),
            int(params->size_k),
            ...
            s_type,
            params->has_bias,
            params->has_act_order,
            params->is_k_full,
            params->has_zp,
            int(params->num_groups),
            ...
            params->thread_n,
            sms,
            params->use_atomic_add,
            params->use_fp32_reduce,
            params->is_zp_float);
...
extern "C" bool ggml_cuda_marlin_w4a16_gemm(
        const ggml_tensor * a,
        const ggml_tensor * b_q_weight,
        const ggml_tensor * b_scales,
        const ggml_tensor * b_zeros,
        ggml_tensor * c,
    ...
    params.b_zeros = b_zeros ? b_zeros->data : nullptr;
