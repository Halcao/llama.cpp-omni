...
void llama_model::load_hparams(llama_model_loader & ml) {
    ...
    switch (arch) {
        case LLM_ARCH_LLAMA:
            {
                // Backward-compatible: layer_norm_rms_epsilon is optional for older GGUF files
                ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps, /*required=*/false);
                // Default value if not present (for backward compatibility with new GGUF files)
                if (hparams.f_norm_rms_eps == 0.0f) {
                    hparams.f_norm_rms_eps = 1e-6f;  // Common default value for RMS normalization
                }
            ...
            case LLM_ARCH_GRANITE_MOE:
                {
                    ...
                    // Skip custom TTS tensors (emb_text, projector_semantic, projector_spk, head_code)
                    // These are loaded separately in omni_init via load_tts_weights_from_gguf
                    // We need to mark them as created to avoid tensor count mismatch
                    ...
                    for (const char * tts_name : tts_tensor_names) {
                        ...
                        if (tts_tensor) {
                            ...
                            LLAMA_LOG_INFO("%s: skipping TTS tensor %s (size = %zu bytes) -- will be loaded separately\n", 
...
struct llm_build_llama : public llm_graph_context {
    llm_build_llama(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        // lm_head
        // Skip output logits computation if output uses token_embd as fallback
        // This happens when GGUF doesn't have output.weight and TTS uses custom head_code
        ...
        ggml_build_forward_expand(gf, cur);
            ...
            // Skip logits computation - TTS uses custom head_code instead
