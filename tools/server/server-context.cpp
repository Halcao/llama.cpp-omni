...
static std::string omni_format_sse(const json & data) {
    return std::string("data: ") + safe_json_to_str(data) + "\n\n";
}
...
void server_routes::init_routes() {
    ...
    // ---------- omni / MiniCPM-o stream API (tools/omni) ----------

    this->post_stream_prefill = [this](const server_http_req & req) -> server_http_res_ptr {
        auto res = create_response(true);
        if (queue_tasks.is_sleeping() || !ctx_server.model || !ctx_server.ctx) {
            res->error(format_error_response("Omni API requires a loaded model", ERROR_TYPE_INVALID_REQUEST));
            return res;
        }
        json data;
        try {
        ...
        if (!data.contains("audio_path_prefix") || !data.at("audio_path_prefix").is_string()) {
            res->error(format_error_response("\"audio_path_prefix\" must be provided as string", ERROR_TYPE_INVALID_REQUEST));
            return res;
        }
        if (!data.contains("cnt") || !data.at("cnt").is_number_integer()) {
            res->error(format_error_response("\"cnt\" must be provided as integer", ERROR_TYPE_INVALID_REQUEST));
            return res;
        }
        {
            std::lock_guard<std::mutex> lock(ctx_server.octx_mutex);
            if (ctx_server.octx == nullptr) {
                res->error(format_error_response("omni context not initialized. call /v1/stream/omni_init first", ERROR_TYPE_INVALID_REQUEST));
                return res;
            }
        }
        ...
        {
            std::lock_guard<std::mutex> lock(ctx_server.octx_mutex);
            if (!stream_prefill(ctx_server.octx, audio_path_prefix, img_fname, cnt, max_slice_nums)) {
                ok = false;
            }
        }
        if (!ok) {
            res->error(format_error_response("stream_prefill failed", ERROR_TYPE_SERVER));
            return res;
        }
        res->ok(json {
            {"success", true},
    ...
    this->post_stream_decode = [this](const server_http_req & req) -> server_http_res_ptr {
        if (queue_tasks.is_sleeping() || !ctx_server.model || !ctx_server.ctx) {
            auto res = create_response(true);
            res->error(format_error_response("Omni API requires a loaded model", ERROR_TYPE_INVALID_REQUEST));
            return res;
        ...
        const std::string debug_dir = data.contains("debug_dir") && data.at("debug_dir").is_string()
                                          ? data.at("debug_dir").get<std::string>()
                                          : std::string("./");
        const bool stream = json_value(data, "stream", true);
        const int  round_idx = json_value(data, "round_idx", -1);
        ...
        if (!stream) {
            ...
            if (!ok) {
                res->error(format_error_response("stream_decode failed", ERROR_TYPE_SERVER));
                return res;
            }
            res->ok(json {{"success", true}, {"debug_dir", debug_dir}});
            return res;
        }
        ...
        struct omni_decode_stream_state {
            ...
            omni_decode_stream_state(server_context_impl & impl_, std::string dd, int ri)
                : impl(impl_), debug_dir(std::move(dd)), round_idx(ri) {}
            ~omni_decode_stream_state() {
                if (worker.joinable()) {
                    worker.join();
                }
            }
            omni_decode_stream_state(const omni_decode_stream_state &)            = delete;
            omni_decode_stream_state & operator=(const omni_decode_stream_state &) = delete;
        };
        auto st = std::shared_ptr<omni_decode_stream_state>(
            new omni_decode_stream_state(ctx_server, debug_dir, round_idx));

        {
            std::lock_guard<std::mutex> lock(ctx_server.octx_mutex);
            std::lock_guard<std::mutex> lock_text(ctx_server.octx->text_mtx);
            ctx_server.octx->text_queue.clear();
            ctx_server.octx->text_done_flag = false;
            ctx_server.octx->text_streaming = true;
        }

        res->next = [st, &req](std::string & output) -> bool {
            try {
                if (req.should_stop()) {
                    if (st->worker.joinable() && !st->worker_joined) {
                        st->worker.join();
                        st->worker_joined = true;
                    }
                    return false;
                }
                if (!st->worker_started) {
                    ...
                    st->worker = std::thread([st] {
                        std::lock_guard<std::mutex> lock(st->impl.octx_mutex);
                        (void) stream_decode(st->impl.octx, st->debug_dir, st->round_idx);
                    });
                }
                while (true) {
                    std::unique_lock<std::mutex> lk(st->impl.octx->text_mtx);
                    st->impl.octx->text_cv.wait_for(lk, std::chrono::milliseconds(200), [&] {
                        return !st->impl.octx->text_queue.empty() || st->impl.octx->text_done_flag;
                    });
                    while (!st->impl.octx->text_queue.empty()) {
                        std::string frag = std::move(st->impl.octx->text_queue.front());
                        st->impl.octx->text_queue.pop_front();
                        lk.unlock();
                        json ev;
                        ...
                        output = omni_format_sse(ev);
                        return true;
                    }
                    if (st->impl.octx->text_done_flag) {
                        break;
                    }
                }
                if (st->worker.joinable() && !st->worker_joined) {
                ...
                output = omni_format_sse(json{{"error", format_error_response(e.what(), ERROR_TYPE_SERVER)}});
                if (st->worker.joinable() && !st->worker_joined) {
                    st->worker.join();
                    st->worker_joined = true;
                }
    ...
    this->post_stream_omni_init = [this](const server_http_req & req) -> server_http_res_ptr {
        auto res = create_response(true);
        if (queue_tasks.is_sleeping() || !ctx_server.model || !ctx_server.ctx) {
            res->error(format_error_response("Omni API requires a loaded model", ERROR_TYPE_INVALID_REQUEST));
            return res;
        ...
        const int   tts_gpu_layers = json_value(data, "tts_gpu_layers", 99);
        ...
        }
        {
            ...
            ctx_server.octx->duplex_mode = duplex_mode;
            if (data.contains("voice_audio") && data.at("voice_audio").is_string()) {
                ...
                if (!voice_audio.empty()) {
                    if (!stream_prefill(ctx_server.octx, voice_audio, "", 0)) {
                        res->error(format_error_response("stream_prefill(voice_audio) failed", ERROR_TYPE_SERVER));
                        return res;
                    }
    ...
    this->post_stream_break = [this](const server_http_req & req) -> server_http_res_ptr {
        auto res = create_response(true);
        if (queue_tasks.is_sleeping()) {
            res->error(format_error_response("Omni API unavailable while server is sleeping", ERROR_TYPE_INVALID_REQUEST));
            return res;
        }
        json data;
        try {
        ...
        }
        {
            ...
            ctx_server.octx->current_turn_ended = true;
            {
                ...
                ctx_server.octx->text_queue.clear();
                ctx_server.octx->text_done_flag = true;
            }
            ctx_server.octx->text_cv.notify_all();
        }
        res->ok(json {{"success", true}, {"message", "generation interrupted"}});
        return res;
    };

    this->post_stream_reset = [this](const server_http_req & req) -> server_http_res_ptr {
        auto res = create_response(true);
        if (queue_tasks.is_sleeping()) {
            res->error(format_error_response("Omni API unavailable while server is sleeping", ERROR_TYPE_INVALID_REQUEST));
            return res;
        ...
        }
        {
            ...
            if (data.contains("duplex_mode")) {
                ctx_server.octx->duplex_mode = data.at("duplex_mode").get<bool>();
            }
        }
        res->ok(json {{"success", true}, {"message", "KV caches cleared, ready for new session"}});
        return res;
    };

    this->post_stream_update_session_config = [this](const server_http_req & req) -> server_http_res_ptr {
        auto res = create_response(true);
        if (queue_tasks.is_sleeping()) {
            res->error(format_error_response("Omni API unavailable while server is sleeping", ERROR_TYPE_INVALID_REQUEST));
            return res;
        ...
        }
        {
            ...
            if (data.contains("media_type")) {
                ctx_server.octx->media_type = data.at("media_type").get<int>();
            }
            if (data.contains("duplex_mode")) {
                ctx_server.octx->duplex_mode = data.at("duplex_mode").get<bool>();
            }
            if (data.contains("sliding_window_mode")) {
                ctx_server.octx->sliding_window_config.mode = data.at("sliding_window_mode").get<std::string>();
            }
            if (data.contains("sliding_window_high_water")) {
                ctx_server.octx->sliding_window_config.high_water_tokens = data.at("sliding_window_high_water").get<int>();
            }
            if (data.contains("sliding_window_low_water")) {
                ctx_server.octx->sliding_window_config.low_water_tokens = data.at("sliding_window_low_water").get<int>();
            }
            if (data.contains("highImage")) {
                ...
                if (ctx_server.octx->ctx_vision) {
                    vision_set_max_slice_nums(ctx_server.octx->ctx_vision, new_high_image ? 2 : -1);
                }
            }
            if (data.contains("highRefresh")) {
                ctx_server.octx->high_refresh = data.at("highRefresh").get<bool>();
            }
            if (ctx_server.octx->ctx_llama) {
                llama_memory_t mem = llama_get_memory(ctx_server.octx->ctx_llama);
                if (mem) {
